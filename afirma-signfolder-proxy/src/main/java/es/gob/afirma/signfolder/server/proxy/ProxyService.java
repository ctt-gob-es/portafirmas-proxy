...
public final class ProxyService extends HttpServlet {
	...
	private static final String[]  OPERATIONS_CREATE_SESSION = new String[] {
	...
	public ProxyService() {
		...
		try {
			...
			LOGGER.warn("No se ha podido recuperar la URL del servicio de firma configurado en la variable " + SIGNATURE_SERVICE_URL + " del sistema: " + e);	 //$NON-NLS-1$ //$NON-NLS-2$
		}
	}
	...
	protected void service(final HttpServletRequest request, final HttpServletResponse response) {
		...
		try {
			...
			// Salvo que sea una operacion de creacion de sesion, permitimos que se nos pase el ID de sesion compartida
			if (!isCreatingLoginOperation(operation)) {
				ssid = request.getParameter(PARAMETER_NAME_SHARED_SESSION_ID);
			}

			// Obtenemos la sesion que corresponda y, si requiere estar validada para realizar
			// la operacion solicitada y no lo esta, se establecera un error de autenticacion
	...
	/**
	 * Indica si una operaci&oacute;n es de las que crean una nueva sesi&oacute;n de usuario.
	 * @param operation C&oacute;digo de la operaci&oacute;n que se quiere comprobar.
	 * @return {@code true} si el c&oacute;digo proporcionado est&aacute; en el listado
	 * de operaciones que crean una nueva sesi&oacute;n, {@code false} en caso contrario.
	 */
	 ...
	 * @param session Sesi&oacute;n pendiente de validar.
	 * @param xml XML con los datos para el proceso de autenticaci&oacute;n.
	 * @return XML con el resultado a la petici&oacute;n.
	 * @throws SAXException Cuando ocurre alg&uacute;n error al procesar los XML.
	 * @throws IOException Cuando ocurre alg&uacute;n problema de comunicaci&oacute;n con el servidor.
	...
	private ValidateLoginResult validateLoginData(final HttpSession session, final ValidateLoginRequest loginRequest) {
		...
		try {
			...
			result.setError(errMsg != null ? errMsg : "El usuario no dispone de cuenta en este portafirmas o utiliz\u00F3 un certificado no v\u00E1lido."); //$NON-NLS-1$
		}
		catch (final Exception e) {
	...
	/**
	 * Procesa el mensaje de error remitido por el Portafirmas para hacerlo legible
	 * para el usuario.
	 * @param rawErrorMessage Mensaje devuelto por el Portafirmas.
	 ...
	 * @param request Petici&oacute;n realizada al servicio.
	 * @param session Ses&iacute;n que pueda haber actualmente iniciada.
	 * @param xml XML con los datos para el proceso de autenticaci&oacute;n.
	 * @return XML con el resultado a la petici&oacute;n.
	 * @throws SAXException Cuando ocurre alg&uacute;n error al procesar los XML.
	 * @throws IOException Cuando ocurre alg&uacute;n problema de comunicaci&oacute;n con el servidor.
	 ...
	 */
	private String processRequestClaveLogin(final HttpServletRequest request,  final HttpSession session, final byte[] xml)
	...
	private static void checkPkcs1(final byte[] pkcs1, final byte[] certEncoded, final byte[] data) throws SignatureException, NoSuchAlgorithmException, CertificateException, InvalidKeyException {
		...
		if (!signer.verify(pkcs1)) {
			throw new SignatureException("La firma no se realizo sobre el token proporcionado o con el certificado indicado"); //$NON-NLS-1$
		}
	}
	 ...
	 */
	private PartialSignRequestsList getRequestsList(final String dni, final ListRequest listRequest) throws MobileException {
		...
		final MobileRequestList mobileRequestsList = getService().queryRequestList(
				dni.getBytes(),
		...
		for (final MobileRequest request : mobileRequestsList.getRequest()) {
			...
			try {
				...
				LOGGER.warn("Se ha encontrado un error al analizar los datos de los documentos de la peticion con ID '" + id + "' y no se mostrara: " + e.toString()); //$NON-NLS-1$ //$NON-NLS-2$
				continue;
			}
	 ...
	 */
	private DocumentData previewDocument(final String dni, final PreviewRequest request) throws MobileException, IOException {
	 ...
	 * @throws IOException Cuando no ha sido posible leer el documento.
	 */
	private DocumentData previewSign(final String dni, final PreviewRequest request) throws MobileException, IOException {
	 ...
	 * @throws IOException Cuando no ha sido posible leer el documento.
	 */
	private DocumentData previewSignReport(final String dni, final PreviewRequest request) throws MobileException, IOException {
	 ...
	 * @throws IOException Cuando no ha sido posible leer el documento.
	 */
	private AppConfiguration loadConfiguration(final String dni, final ConfigurationRequest request) throws MobileException, IOException {
		...
		for (final TriphaseRequest singleRequest : triRequests) {
			...
			try {
				...
				if (singleRequest.size() != downloadedDocs.getDocument().size()) {
					throw new Exception("No se han recuperado tantos documentos como los indicados en la peticion " + singleRequest.getRef()); //$NON-NLS-1$
				}
					...
					for (final MobileDocument downloadedDoc : downloadedDocs.getDocument()) {
						if (downloadedDoc.getIdentifier().equals(docRequest.getId())) {
							...
							if (content instanceof InputStream) {
								...
								throw new IOException("El tipo con el que se devuelve el contenido del documento no esta soportado: " + //$NON-NLS-1$
										(content == null ? null : content.getClass()));
							}
							break;
						}
		...
		for (final TriphaseRequest triRequest : triRequests) {
			...
			if (!requestNeedContent.isEmpty()) {
				try {
					...
					LOGGER.warn("Ocurrio un error al descargar los documentos de la peticion " + triRequest.getRef() + ": " + ex);  //$NON-NLS-1$//$NON-NLS-2$
	...
	/**
	 * Obtiene el servicio para realizar las peticiones al Portafirmas web. Si se tienen habilitadas las opciones
	 * de depuraci&oacute;n, se desactivan las comprobaciones SSL.
